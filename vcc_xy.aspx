		...
		<script type="text/javascript">
				...
				switch(q_getPara('sys.project').toUpperCase()){
					case 'XY':
						...
						$('#btnPrint2').click(function() {
							q_box("z_vccap.aspx?" + r_userno + ";" + r_name + ";" + q_time + ";" + JSON.stringify({noa:trim($('#txtInvono').val())}) + ";" + r_accy + "_" + r_cno, 'vcca', "95%", "95%", q_getMsg("popPrint"));
						});
						break;
				}
				
				if(r_rank<'8'){
					q_readonly.push('txtNick');
			...
			function _btnSeek() {
				if (q_cur > 0 && q_cur < 4)// 1-3
					return;
				q_box('vcc_xy_s.aspx', q_name + '_s', "720px", "570px", q_getMsg("popSeek"));
			}
				...
				for (var i = 0; i < q_bbsCount; i++) {
					if (!$('#btnMinus_' + i).hasClass('isAssign')) {
						...
						$('#cmbItemno_' + i).change(function() {
							...
							var tmemo=$('#txtMemo_'+b_seq).val().split('[')[0];
							if(t_max_unit!=t_unit && Math.floor(t_mount/t_max_inmout)>0){
								...
								if($('#cmbItemno_' + b_seq).val()=='0')
									$('#txtMemo_'+b_seq).val(tmemo+'['+t_m1+t_max_unit+(t_m2>0?('+'+t_m2+t_unit):'')+']');
								else
									$('#txtMemo_'+b_seq).val(tmemo+'['+$('#cmbItemno_' + b_seq).find("option:selected").text()+'：'+t_m1+t_max_unit+(t_m2>0?('+'+t_m2+t_unit):'')+']');
							}else{
								if($('#cmbItemno_' + b_seq).val()!='0'){
									$('#txtMemo_'+b_seq).val(tmemo+'['+$('#cmbItemno_' + b_seq).find("option:selected").text()+'：'+dec($('#txtDime_'+b_seq).val())+t_unit+']');
								}else{
									$('#txtMemo_'+b_seq).val(tmemo);
								}
							}
				...
				if(q_cur==2){
					if($('#txtInvono').val().length>0 && $('#cmbTranstyle').val()!='' && r_rank<5){
						for (var i=0;i<fbbm.length;i++){
							...
							//106/11/06 M02開放可改營業稅別 //106/12/22 開放M20
							if(r_userno.toUpperCase()=='M02' || r_userno.toUpperCase()=='M20'){
